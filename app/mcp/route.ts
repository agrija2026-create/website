    ...
    server.registerTool(
      "rice_advance_payment",
      {
        ...
        description:
          ...
          "confirmation は情報源の区分で、決定＝JA・全農が決定を自ら発表・公表した金額（announcedOn は発表日）、報道＝報道機関の取材で判明しJA・全農は金額を公表していない金額（announcedOn は金額が明らかになった日）。報道もJAが決めた金額であり未決定ではない。回答では両者を区別すること。" +
