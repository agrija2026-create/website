...
### 1) このチャットで指示して公開（おすすめ）

あなたはこのチャットで、次を送るだけでOKです。

- `title`
- `slug`（英数字とハイフン）
- `description`
- `publishedAt`（YYYY-MM-DD）
- `category`（例: `seisaku`, `yosan`）
- `tags`（例: `smart-agriculture, maff`）
- 本文HTML（`<article>...</article>`）

「この内容で公開して」と指示してもらえれば、こちらで `content/articles/<slug>.md` を作成して push まで進めます。

### 2) 自分で公開する場合（1コマンド）

1. `content/articles/_TEMPLATE.md` をコピーして記事を作成
2. 次を実行

```bash
bash scripts/publish-article.sh <slug>
```
...
これで `content/articles/<slug>.md` の変更をコミットし、`git push` まで実行します。
