...
- 読後に判断・実行できること:
- 主想定KW:
- 検索意図（対象確認／金額確認／申請・実行／比較など）:

### 重複確認と制作方針
- 検索語（正式名称・通称・困りごと・主想定KW）:
- 公開記事・source-htmlの確認結果:
- 作業フォルダ・下書きの確認結果:
- 制作方針（新規／既存増補／統合／棲み分け／下書き継続）:
- 判断理由:
- 既存記事がある場合のユーザー承認:
...
### 見出し・回答・根拠の対応
| 見出し案 | 先に答える内容 | 根拠となる一次資料名・ページまたは公式URL |
|---|---|---|
|  |  |  |

### 文脈内内部リンク
| リンク元の文脈 | リンク先タイトル・URL | つなぐ理由 |
|---|---|---|
|  |  |  |
...
## 次の手順
1. 公開記事・公開本文・作業フォルダを検索し、重複確認と制作方針を記入する
2. SEO記事設計の各欄を埋め、主要見出しと一次資料の根拠を対応させる
3. \`docs/記事生成プロンプト.md\` を開き、制約に沿って \`${ARTICLE_HTML}\` を編集して記事本文を完成させる
4. 特に \`data:image\` / Base64 画像 / 巨大な inline style を含めていないことを確認する
5. 完成後、確認OKなら \`bash scripts/stage-article-for-web.sh "${WORKDIR}" <slug>\` を実行する
6. 最後に \`bash scripts/publish-article.sh <slug>\` で公開する
